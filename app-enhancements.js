...
function executiveAreaLabel(process){var labels={'Processo 3':'Processo 3 — Carregamento / Tratamento / Vazão','Processo 4':'Processo 4 — Roda / Calha / Laminador'};return labels[process]||process}
function executivePeriodValue(row){var month=MONTHS.indexOf(row.month);return(month<0?99:month)*10+Number(row.week||0)}
...
function executivePersonName(value){return String(value||'').replace(/^Equipe\s*\d+\s*:\s*/i,'').trim()}
function drawCompactRanking(ctx,x,y,w,h,title,subtitle,rows){
  ctx.fillStyle='#fff';ctx.strokeStyle='#cbd5e1';ctx.lineWidth=1;ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,w,h,9):ctx.rect(x,y,w,h);ctx.fill();ctx.stroke();
  var titleW=150,itemW=(w-titleW)/10,visible=rows.slice(0,10);ctx.fillStyle='#172033';ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,titleW,h,[9,0,0,9]):ctx.rect(x,y,titleW,h);ctx.fill();ctx.fillStyle='#fff';ctx.font='700 15px Arial';ctx.textAlign='left';ctx.fillText(title,x+14,y+23);ctx.fillStyle='#cbd5e1';ctx.font='9px Arial';ctx.fillText(subtitle,x+14,y+39);
  for(var index=0;index<10;index++){var row=visible[index],left=x+titleW+index*itemW;if(index){ctx.strokeStyle='#e2e8f0';ctx.beginPath();ctx.moveTo(left,y+7);ctx.lineTo(left,y+h-7);ctx.stroke()}if(index<3&&row){ctx.fillStyle=index===0?'#fef9c3':index===1?'#ecfdf5':'#fff7ed';ctx.fillRect(left+1,y+1,itemW-2,h-2)}if(!row)continue;ctx.fillStyle='#166534';ctx.font='700 10px Arial';ctx.fillText((index+1)+'º',left+8,y+15);ctx.fillStyle='#172033';ctx.font='700 10px Arial';var name=executivePersonName(row.team)+(row.count>1?' ('+row.count+')':'');ctx.fillText(fitCanvasText(ctx,name,itemW-14),left+8,y+31);ctx.fillStyle='#334155';ctx.font='10px Arial';ctx.fillText(row.avg.toFixed(2).replace('.',',')+' %',left+8,y+45)}
}
function drawExecutiveArea(ctx,group,x,y,w,h){
  var rows=group.rows,avg=rows.reduce(function(sum,row){return sum+Number(row.score||0)},0)/rows.length,teams=[],latest=rows.slice().sort(function(a,b){return executivePeriodValue(b)-executivePeriodValue(a)||Number(b.id||0)-Number(a.id||0)})[0];rows.forEach(function(row){if(teams.indexOf(row.team)<0)teams.push(row.team)});
  ctx.fillStyle='#fff';ctx.strokeStyle='#cbd5e1';ctx.lineWidth=1;ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,w,h,10):ctx.rect(x,y,w,h);ctx.fill();ctx.stroke();ctx.fillStyle='#172033';ctx.font='700 17px Arial';ctx.textAlign='left';ctx.fillText(fitCanvasText(ctx,executiveAreaLabel(group.process),w-38),x+17,y+25);ctx.fillStyle='#64748b';ctx.font='11px Arial';ctx.fillText('Média '+avg.toFixed(2).replace('.',',')+'%  |  Última '+Number(latest&&latest.score||0).toFixed(2).replace('.',',')+'%  |  Cobertura '+teams.length+' equipe(s)',x+17,y+43);
  var chart=executiveDashboardChart(rows);ctx.drawImage(chart,x+10,y+49,w-20,h-57);
}
function executiveDashboardChart(rows){
  var canvas=document.createElement('canvas'),ctx=canvas.getContext('2d'),W=1400,H=590,left=78,right=W-20,top=27,bottom=H-112,palette=['#dc2626','#2563eb','#16a34a','#d97706','#7c3aed','#0891b2','#db2777','#65a30d'],teams=[];canvas.width=W;canvas.height=H;ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);rows.slice().sort(function(a,b){return executivePeriodValue(a)-executivePeriodValue(b)}).forEach(function(row){if(teams.indexOf(row.team)<0)teams.push(row.team)});var data=teams.map(function(team,index){return{name:executivePersonName(team),color:palette[index%palette.length],values:rows.filter(function(row){return row.team===team}).sort(function(a,b){return executivePeriodValue(a)-executivePeriodValue(b)||Number(a.id||0)-Number(b.id||0)}).slice(-6)}}),plotH=bottom-top,count=Math.max.apply(null,data.map(function(team){return team.values.length})),barAreaLeft=left+95,groupW=(right-barAreaLeft)/Math.max(data.length,1),gap=4,barW=Math.max(10,Math.min(42,(groupW-22-(count-1)*gap)/Math.max(count,1)));function py(value){return bottom-Math.max(0,Math.min(100,Number(value||0)))/100*plotH}for(var tick=0;tick<=100;tick+=20){var y=py(tick);ctx.strokeStyle='#e5e7eb';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(right,y);ctx.stroke();ctx.fillStyle='#94a3b8';ctx.font='11px Arial';ctx.textAlign='right';ctx.fillText(tick+'%',left-8,y+4)}[[95,'#22c55e','>95% EXCELENTE'],[80,'#3b82f6','80% BOM'],[60,'#ef4444','<60% CRÍTICO']].forEach(function(rule){var y=py(rule[0]);ctx.strokeStyle=rule[1];ctx.lineWidth=2;ctx.setLineDash([8,5]);ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(right,y);ctx.stroke();ctx.setLineDash([]);ctx.font='700 10px Arial';var tagW=ctx.measureText(rule[2]).width+12;ctx.fillStyle=rule[1];ctx.fillRect(left+2,y-15,tagW,14);ctx.fillStyle='#fff';ctx.textAlign='left';ctx.fillText(rule[2],left+8,y-4)});data.forEach(function(team,teamIndex){var groupX=barAreaLeft+teamIndex*groupW,barsW=team.values.length*barW+(team.values.length-1)*gap,start=groupX+(groupW-barsW)/2,points=[];team.values.forEach(function(row,index){var value=Number(row.score||0),x=start+index*(barW+gap),y=py(value),color=value>95?'#16a34a':value>=80?'#3b82f6':value>=60?'#eab308':'#dc2626';ctx.fillStyle=color;ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,barW,bottom-y,[3,3,0,0]):ctx.rect(x,y,barW,bottom-y);ctx.fill();points.push({x:x+barW/2,y:y});ctx.fillStyle='#172033';ctx.font='700 11px Arial';ctx.textAlign='center';ctx.fillText(value.toFixed(1).replace('.',','),x+barW/2,Math.max(top+11,y-5));ctx.font='800 13px Arial';ctx.fillText(String(index+1)+'ª',x+barW/2,bottom-7)});ctx.strokeStyle=team.color;ctx.lineWidth=2.5;ctx.beginPath();points.forEach(function(point,index){if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y)});ctx.stroke();points.forEach(function(point){ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(point.x,point.y,4,0,Math.PI*2);ctx.fill();ctx.strokeStyle=team.color;ctx.lineWidth=2;ctx.stroke()});ctx.fillStyle=team.color;ctx.font='700 12px Arial';ctx.textAlign='center';ctx.fillText(fitCanvasText(ctx,team.name,groupW-8),groupX+groupW/2,bottom+19);if(team.values.length>1){var delta=Number(team.values[team.values.length-1].score)-Number(team.values[0].score),positive=delta>=0;ctx.fillStyle=positive?'#16a34a':'#dc2626';ctx.font='700 10px Arial';ctx.fillText((positive?'▲ +':'▼ ')+delta.toFixed(1).replace('.',',')+' pts',groupX+groupW/2,bottom+35)}});var legend=[['#16a34a','>95 Excelente'],['#3b82f6','80–95 Bom'],['#eab308','60–80 Regular'],['#dc2626','<60 Crítico']],legendX=left;legend.forEach(function(item){ctx.fillStyle=item[0];ctx.fillRect(legendX,H-22,10,10);ctx.fillStyle='#475569';ctx.font='10px Arial';ctx.textAlign='left';ctx.fillText(item[1],legendX+15,H-13);legendX+=ctx.measureText(item[1]).width+48});ctx.fillStyle='#64748b';ctx.textAlign='right';ctx.fillText('Linha: evolução da equipe',right,H-13);return canvas
}
function executivePageCanvas(scores,groups,pageIndex,totalPages){
  ...
  drawCompactRanking(ctx,58,108,W-116,56,'Mensal — '+MONTHS[cpm],'TOP 10',reportRankingAllPeople(scores,MONTHS[cpm]));drawCompactRanking(ctx,58,174,W-116,56,'Anual — 2026','TOP 10',reportRankingAllPeople(scores,null));
  var gap=18,gridX=58,gridY=250,cardW=(W-116-gap)/2,cardH=437;groups.forEach(function(group,index){drawExecutiveArea(ctx,group,gridX+(index%2)*(cardW+gap),gridY+Math.floor(index/2)*(cardH+gap),cardW,cardH)});
...
window.generateExecutivePdf=async function(){var scores=gas(),button=$('executive-pdf-button');if(!scores.length){toast('⚠️ Não há processos alimentados para gerar a apresentação.');return}if(!window.jspdf||!jspdf.jsPDF){toast('❌ O gerador de PDF não foi carregado. Atualize a página e tente novamente.');return}button.disabled=true;button.textContent='Gerando apresentação...';var today=new Date(),fileDate=today.getFullYear()+'-'+String(today.getMonth()+1).padStart(2,'0')+'-'+String(today.getDate()).padStart(2,'0')+'_'+String(today.getHours()).padStart(2,'0')+String(today.getMinutes()).padStart(2,'0');try{await generatePagedExecutivePdf(scores,fileDate);toast('✅ Apresentação gerada com '+executiveProcessGroups(scores).length+' área(s) em múltiplas folhas.')}catch(error){console.error(error);toast('❌ Não foi possível gerar a apresentação: '+error.message)}finally{button.disabled=false;button.textContent='📄 Extrair apresentação A4 (PDF)'}};
...
function installAuditFormButtons(){Object.keys(app.types).forEach(function(type){var config=app.types[type],sheet=$(config.sheet),actions=sheet&&sheet.querySelector('.form-actions'),table=sheet&&sheet.querySelector('.audit-table');if(!actions||!table)return;if(!actions.querySelector('[data-form-editor]')){var button=document.createElement('button');button.type='button';button.className='btn form-editor-button no-print';button.dataset.formEditor=type;button.textContent='⚙️ Editar perguntas e pesos';button.onclick=function(){window.openAuditFormEditor(type)};actions.insertBefore(button,actions.firstChild)}if(!sheet.querySelector('.form-editor-toolbar-top')){var toolbar=document.createElement('div');toolbar.className='form-editor-toolbar-top no-print';toolbar.innerHTML='<span><strong>Formulário configurável</strong><small>Adicione, remova ou altere perguntas e pesos deste setor.</small></span><button type="button" class="btn form-editor-button">⚙️ Editar perguntas e pesos</button>';toolbar.querySelector('button').onclick=function(){window.openAuditFormEditor(type)};table.insertAdjacentElement('beforebegin',toolbar)}})}
function formEditorRow(item,index){return'<div class="form-editor-row" data-index="'+index+'"><div class="form-editor-order"><button type="button" class="form-editor-drag-handle" title="Arraste para alterar a posição" aria-label="Arrastar pergunta '+(index+1)+'"><b>⠿</b><span>ARRASTAR</span></button><button type="button" data-move="up" title="Mover para cima">↑</button><button type="button" data-move="down" title="Mover para baixo">↓</button><strong>'+(index+1)+'</strong></div><label>GRUPO<input data-field="grupo" maxlength="120" value="'+html(item.grupo)+'"></label><label class="form-editor-question">PERGUNTA<textarea data-field="pergunta" maxlength="1000">'+html(item.pergunta)+'</textarea></label><label>PESO (OK)<input data-field="peso" type="number" min="0.001" max="1000" step="0.001" value="'+html(auditWeightLabel(item.peso))+'"></label><label class="form-editor-parameter">PARÂMETRO<textarea data-field="parametro" maxlength="2000">'+html(item.parametro||'')+'</textarea></label><button type="button" class="form-editor-remove" title="Remover pergunta">✕</button></div>'}
...
var formEditorDrag=null;
function finishFormEditorDrag(event){if(!formEditorDrag||event.pointerId!==formEditorDrag.pointerId)return;var drag=formEditorDrag,list=drag.row.parentElement;try{drag.handle.releasePointerCapture(event.pointerId)}catch(_){ }drag.row.classList.remove('dragging');list.classList.remove('sorting');formEditorDrag=null;if(!drag.moved)return;var current=formEditorItems.slice(),ordered=Array.from(list.querySelectorAll('.form-editor-row')).map(function(row){return current[Number(row.dataset.index)]}).filter(Boolean);if(ordered.length===current.length)formEditorItems=ordered;renderFormEditor();$('audit-form-editor-count').textContent=formEditorItems.length+' pergunta(s) • ordem atualizada automaticamente'}
function moveFormEditorDrag(event){if(!formEditorDrag||event.pointerId!==formEditorDrag.pointerId)return;var drag=formEditorDrag,list=drag.row.parentElement;if(!drag.moved&&Math.abs(event.clientY-drag.startY)<5)return;drag.moved=true;var listRect=list.getBoundingClientRect();if(event.clientY<listRect.top+38)list.scrollTop-=16;else if(event.clientY>listRect.bottom-38)list.scrollTop+=16;var target=document.elementFromPoint(event.clientX,event.clientY),targetRow=target&&target.closest('.form-editor-row');if(targetRow&&targetRow!==drag.row&&targetRow.parentElement===list){var rect=targetRow.getBoundingClientRect();list.insertBefore(drag.row,event.clientY<rect.top+rect.height/2?targetRow:targetRow.nextSibling)}event.preventDefault()}
function startFormEditorDrag(event){if(event.button!=null&&event.button!==0)return;var handle=event.currentTarget,row=handle.closest('.form-editor-row'),list=row&&row.parentElement;if(!row||!list)return;syncFormEditorState();formEditorDrag={row:row,handle:handle,pointerId:event.pointerId,startY:event.clientY,moved:false};row.classList.add('dragging');list.classList.add('sorting');try{handle.setPointerCapture(event.pointerId)}catch(_){ }event.preventDefault()}
function renderFormEditor(){var list=$('audit-form-editor-list');if(!list)return;list.innerHTML=formEditorItems.map(formEditorRow).join('');list.querySelectorAll('.form-editor-drag-handle').forEach(function(handle){handle.addEventListener('pointerdown',startFormEditorDrag);handle.addEventListener('pointermove',moveFormEditorDrag);handle.addEventListener('pointerup',finishFormEditorDrag);handle.addEventListener('pointercancel',finishFormEditorDrag)});list.querySelectorAll('[data-move]').forEach(function(button){button.onclick=function(){syncFormEditorState();var index=Number(button.closest('.form-editor-row').dataset.index),target=button.dataset.move==='up'?index-1:index+1;if(target<0||target>=formEditorItems.length)return;var item=formEditorItems[index];formEditorItems[index]=formEditorItems[target];formEditorItems[target]=item;renderFormEditor()}});list.querySelectorAll('.form-editor-remove').forEach(function(button){button.onclick=function(){syncFormEditorState();formEditorItems.splice(Number(button.closest('.form-editor-row').dataset.index),1);renderFormEditor()}});$('audit-form-editor-count').textContent=formEditorItems.length+' pergunta(s)'}
function ensureFormEditorModal(){if($('audit-form-editor'))return;var modal=document.createElement('div');modal.id='audit-form-editor';modal.className='audit-form-editor-overlay';modal.innerHTML='<div class="audit-form-editor-card"><header><div><span>CONFIGURAÇÃO DO FORMULÁRIO</span><h2 id="audit-form-editor-title">Perguntas e pesos</h2><p>O peso informado é o valor obtido quando a resposta for OK; respostas NÃO recebem zero.</p></div><button type="button" class="report-close" onclick="closeAuditFormEditor()">Fechar</button></header><div class="audit-form-editor-toolbar"><button type="button" class="btn btn-save" onclick="addAuditFormQuestion()">＋ Adicionar pergunta</button><button type="button" class="btn btn-clear" onclick="restoreDefaultAuditForm()">↺ Restaurar modelo padrão</button><small class="form-editor-drag-help">⠿ Arraste as perguntas. A numeração é reorganizada automaticamente.</small><span id="audit-form-editor-count"></span></div><div id="audit-form-editor-list" class="audit-form-editor-list"></div><footer><button type="button" class="btn btn-clear" onclick="closeAuditFormEditor()">Cancelar</button><button type="button" class="btn btn-save" id="audit-form-editor-save" onclick="saveAuditFormConfiguration()">💾 Salvar formulário</button></footer></div>';document.body.appendChild(modal)}
