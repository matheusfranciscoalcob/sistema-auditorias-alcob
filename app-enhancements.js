...
function drawExecutiveArea(ctx,group,x,y,w,h){
  ...
  ctx.fillStyle='#fff';ctx.strokeStyle='#cbd5e1';ctx.lineWidth=1;ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,w,h,10):ctx.rect(x,y,w,h);ctx.fill();ctx.stroke();ctx.fillStyle='#166534';ctx.fillRect(x,y,6,h);ctx.fillStyle='#172033';ctx.font='700 18px Arial';ctx.textAlign='left';ctx.fillText(fitCanvasText(ctx,executiveAreaLabel(group.process),w-190),x+20,y+28);ctx.fillStyle='#166534';ctx.font='700 17px Arial';ctx.textAlign='right';ctx.fillText(avg.toFixed(2).replace('.',',')+'%',x+w-18,y+28);ctx.fillStyle='#64748b';ctx.font='11px Arial';ctx.fillText(teams.length+' auditado(s) • '+rows.length+' registro(s)',x+w-18,y+46);ctx.textAlign='left';
  var chart=executiveDashboardChart(rows);ctx.drawImage(chart,x+18,y+57,w-34,h-72);
}
function executiveDashboardChart(rows){
  var canvas=document.createElement('canvas'),ctx=canvas.getContext('2d'),W=1400,H=510,left=62,right=W-22,top=34,bottom=H-78,palette=['#dc2626','#2563eb','#16a34a','#d97706','#7c3aed','#0891b2','#db2777','#65a30d'],teams=[];canvas.width=W;canvas.height=H;ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);rows.slice().sort(function(a,b){return executivePeriodValue(a)-executivePeriodValue(b)}).forEach(function(row){if(teams.indexOf(row.team)<0)teams.push(row.team)});var data=teams.map(function(team,index){return{name:String(team).replace(/^Equipe\s*\d+\s*:\s*/i,''),color:palette[index%palette.length],values:rows.filter(function(row){return row.team===team}).sort(function(a,b){return executivePeriodValue(a)-executivePeriodValue(b)||Number(a.id||0)-Number(b.id||0)}).slice(-6)}}),plotW=right-left,plotH=bottom-top,count=Math.max.apply(null,data.map(function(team){return team.values.length})),groupW=plotW/Math.max(data.length,1),gap=4,barW=Math.max(10,Math.min(38,(groupW-20-(count-1)*gap)/Math.max(count,1)));function py(value){return bottom-Math.max(0,Math.min(100,Number(value||0)))/100*plotH}for(var tick=0;tick<=100;tick+=20){var y=py(tick);ctx.strokeStyle='#edf2f7';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(right,y);ctx.stroke();ctx.fillStyle='#94a3b8';ctx.font='11px Arial';ctx.textAlign='right';ctx.fillText(tick+'%',left-7,y+4)}[[95,'#22c55e'],[80,'#3b82f6'],[60,'#ef4444']].forEach(function(rule){var y=py(rule[0]);ctx.strokeStyle=rule[1];ctx.lineWidth=2;ctx.setLineDash([7,4]);ctx.beginPath();ctx.moveTo(left,y);ctx.lineTo(right,y);ctx.stroke();ctx.setLineDash([])});data.forEach(function(team,teamIndex){var groupX=left+teamIndex*groupW,barsW=team.values.length*barW+(team.values.length-1)*gap,start=groupX+(groupW-barsW)/2,points=[];team.values.forEach(function(row,index){var value=Number(row.score||0),x=start+index*(barW+gap),y=py(value),color=value>95?'#16a34a':value>=80?'#3b82f6':value>=60?'#eab308':'#dc2626';ctx.fillStyle=color;ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,barW,bottom-y,[3,3,0,0]):ctx.rect(x,y,barW,bottom-y);ctx.fill();points.push({x:x+barW/2,y:y});ctx.fillStyle='#172033';ctx.font='700 11px Arial';ctx.textAlign='center';ctx.fillText(value.toFixed(1).replace('.',','),x+barW/2,Math.max(top+10,y-5));ctx.font='800 13px Arial';ctx.fillText(String(index+1),x+barW/2,y+16)});ctx.strokeStyle=team.color;ctx.lineWidth=2.5;ctx.beginPath();points.forEach(function(point,index){if(index)ctx.lineTo(point.x,point.y);else ctx.moveTo(point.x,point.y)});ctx.stroke();points.forEach(function(point){ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(point.x,point.y,4,0,Math.PI*2);ctx.fill();ctx.strokeStyle=team.color;ctx.lineWidth=2;ctx.stroke()});ctx.fillStyle=team.color;ctx.font='700 12px Arial';ctx.textAlign='center';ctx.fillText(fitCanvasText(ctx,team.name,groupW-8),groupX+groupW/2,bottom+20);if(team.values.length>1){var delta=Number(team.values[team.values.length-1].score)-Number(team.values[0].score),positive=delta>=0;ctx.fillStyle=positive?'#16a34a':'#dc2626';ctx.font='700 10px Arial';ctx.fillText((positive?'▲ +':'▼ ')+delta.toFixed(1).replace('.',',')+' pts',groupX+groupW/2,bottom+36)}});return canvas
}
